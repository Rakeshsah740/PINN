...
class PhysicsInformedNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        alpha_raw = nn.Dense(features=1)(phys)
        alpha = 0.01 + 0.98 * jax.nn.sigmoid(alpha_raw)   # (0.01, 0.99), avoids exact 0/1
        
        raw_b = nn.Dense(features=1)(phys)
        b = 0.01 + 0.6 * jax.nn.sigmoid(raw_b)   # bounded to roughly (0.01, 0.61)

        return nn_pred, alpha, b
...
def mse_loss(params, model, x, y):
    logN_pred, _, _ = model.apply(params, x)
    return jnp.mean((logN_pred - y) ** 2)

  
def physics_loss(params, model, x, sigma_a_raw,sigma_a_std, Rm):
    logN, alpha, b = model.apply(params, x)
    N = 10**logN
    denominator = (1 - alpha + alpha * N)**b
    phy_calc = Rm / denominator
    residual = (sigma_a_raw - phy_calc) / sigma_a_std
    return jnp.mean(residual**2)

    

def total_loss(params, model, x, sigma_a_raw, y, lambda_phys,sigma_a_std, Rm):
    mse = mse_loss(params, model, x, y)
    phys = physics_loss(params, model, x,sigma_a_raw,sigma_a_std, Rm)
...
def train_pinn_sendeckyj(data_path, num_epochs=650, lr=0.001, lamb=1e-4, random_state=42):
    ...
    Rm = df['Rm'].values.astype(float).reshape(-1, 1)

    X_train_np, X_test_np, y_train_np, y_test_np, Rm_train_np, Rm_test_np = train_test_split(
        X, y, Rm, test_size=0.2, random_state=random_state
    )
    ...
    Rm_train = jnp.array(Rm_train_np)
    Rm_test = jnp.array(Rm_test_np)
    ...
    def train_step(params, opt_state, x, sigma_a_raw, y):
        loss, grads = jax.value_and_grad(
            lambda p: total_loss(p, model, x, sigma_a_raw, y, lamb,sigma_a_std, Rm_train)
        )(params)
    ...
    for epoch in pbar:
        ...
        if epoch % 20 == 0 or epoch == 1:
            ...
            phys_loss_value = physics_loss(params,model, X_train, sigma_a_train_raw,sigma_a_std, Rm_train)
            nn_loss_value = mse_loss(params,model, X_train, y_train)
            total_loss_value = total_loss(params,model, X_train, sigma_a_train_raw, y_train, lamb,sigma_a_std, Rm_train)
